...
class DNSGlobalScanner(ThreadPool):
	...
	def dns_lookup(self, ip, nameserver=None):
		try:
			...
			return [str(hostname).strip('.')
				for hostname in r.resolve_address(ip)]
	...
	def dns_resolve(self, hostname, nameserver=None):
		try:
			...
			if nameserver:
				r.nameservers = [nameserver]
			return {str(ip): self.dns_lookup(str(ip), nameserver)
				for ip in r.query(hostname)}
		except dns.exception.DNSException:
			return {}
	...
	def handler(self, nameserver):
		log.debug(f"{nameserver['ip_address']} - "
		          f"{nameserver['as_org']} - "
		          f"[{nameserver['country_name']} / "
		          f"{nameserver['country_code']}]")
		nameserver['domain'] = self.domain
		nameserver['answers'] = self.dns_resolve(
			self.domain, nameserver['ip_address'])
